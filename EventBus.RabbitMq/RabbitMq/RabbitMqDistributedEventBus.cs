...
using Uow;
using NullDisposable = Core.NullDisposable;

namespace EventBus.RabbitMq.RabbitMq;

public class RabbitMqDistributedEventBus : DistributedEventBusBase
{
    protected EqnRabbitMqEventBusOptions EqnRabbitMqEventBusOptions { get; }
    ...
    public RabbitMqDistributedEventBus(
        IOptions<EqnRabbitMqEventBusOptions> options,
            ...
            eventHandlerInvoker)
    {
        ...
        EqnRabbitMqEventBusOptions = options.Value;
    ...
    public void Initialize()
    {
        Consumer = MessageConsumerFactory.Create(
            new ExchangeDeclareConfiguration(
                EqnRabbitMqEventBusOptions.ExchangeName,
                type: EqnRabbitMqEventBusOptions.GetExchangeTypeOrDefault(),
                durable: true
            ),
            new QueueDeclareConfiguration(
                EqnRabbitMqEventBusOptions.ClientName,
                durable: true,
                exclusive: false,
                autoDelete: false,
                prefetchCount: EqnRabbitMqEventBusOptions.PrefetchCount
            ),
            EqnRabbitMqEventBusOptions.ConnectionName
        );

        Consumer.OnMessageReceived(ProcessEventAsync);

        SubscribeHandlers(EqnDistributedEventBusOptions.Handlers);
    }
    ...
    public async override Task PublishManyFromOutboxAsync(
        ...
        OutboxConfig outboxConfig)
    {
        using (var channel = ConnectionPool.Get(EqnRabbitMqEventBusOptions.ConnectionName).CreateModel())
        {
        ...
        Guid? eventId = null)
    {
        ...
        channel.BasicPublish(
            exchange: EqnRabbitMqEventBusOptions.ExchangeName,
    ...
    private void EnsureExchangeExists(IModel channel)
    {
        ...
        try
        {
            channel.ExchangeDeclarePassive(EqnRabbitMqEventBusOptions.ExchangeName);
        }
        catch (Exception)
        {
            channel.ExchangeDeclare(
                EqnRabbitMqEventBusOptions.ExchangeName,
                EqnRabbitMqEventBusOptions.GetExchangeTypeOrDefault(),
                durable: true
            );
        }
